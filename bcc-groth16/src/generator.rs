use crate::{r1cs_to_qap::R1CSToQAP, BccGroth16, CommittingKey, ProvingKey, Vec, VerifyingKey};
...
impl<E: Pairing, QAP: R1CSToQAP, const M: usize> BccGroth16<E, QAP, M> {
        ...
        C: ConstraintSynthesizer<E::ScalarField>,
    {
        ...
        let (one, gamma_abc_g1) = gamma_abc_g1.split_first().unwrap();
        let (batched, gamma_abc_g1) = gamma_abc_g1.split_at(M + 1);
        let (tau, gamma_abc_g1) = gamma_abc_g1.split_last().unwrap();
        let proof_dependent = [gamma_abc_g1, &[gamma_eta_g1][..]].concat();
        let gamma_abc_g1 = [&[*one][..], &[*tau][..]].concat();

        // Convert vec to slice
        let proof_dependent = proof_dependent.as_slice();
        let gamma_abc_g1 = gamma_abc_g1.as_slice();
        ...
        let ck = CommittingKey {
            batched: E::G1::normalize_batch(batched),
            proof_dependent: E::G1::normalize_batch(proof_dependent),
        };
