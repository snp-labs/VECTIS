...
use crate::poly_commit::{
    convert_to_bigints, skip_leading_zeros_and_convert_to_bigints, BTreeMap, Error,
    LabeledPolynomial, PCRandomness, ToString, Vec,
};
    ...
    for<'a, 'b> &'a P: Div<&'b P, Output = P>,
{
    ...
    pub fn commit_with_lagrange(
        ...
        assert_eq!(
            blind_of_g.len(),
            opening.len(),
            "Opening length does not match the exepcted blinding factors"
        );
        ...
        assert_eq!(
            bck_2[..n].len(),
            committed_witness.len(),
            "Committed-witness length does not match the exepcted blinding factors"
        );
