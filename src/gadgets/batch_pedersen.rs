...
mod test {
    ...
    fn aggregate_variable_vector<F, P>(
        ...
        where F: PrimeField, P: TEModelParameters<BaseField = F>
    {
        ...
        while sz > 1 {
            ...
            for i in (1..var_cp_vec.len()).step_by(2) {
                sum_var = composer.arithmetic_gate(|gate| {
                    gate.witness(sum_var, var_cp_vec[i], None).add(F::one(), F::one())
                });
            }

            sum_var = composer.arithmetic_gate(|gate| {
                gate.witness(sum_var, coeff, None).mul(F::one())
            });

            agg = composer.arithmetic_gate(|gate| {
                gate.witness(agg, sum_var, None).add(F::one(), F::one())
            });

            // coeff = coeff * coeff
            coeff = composer.arithmetic_gate(|gate| {
                gate.witness(coeff, coeff, None).mul(F::one())
            });
            ...
            for i in 0..sz {
                var_cp_vec[i] = composer.arithmetic_gate(|gate| {
                    gate.witness(var_cp_vec[i << 1], var_cp_vec[(i << 1) + 1], None).add(
                        F::one(),
                        F::one()
                    )
                });
            ...
            VerifierData<F, PC>: PartialEq
    {
        ...
        let deserialized_verifier_data: VerifierData<F, PC> = VerifierData::deserialize(
            verifier_data_bytes.as_slice()
        ).unwrap();
