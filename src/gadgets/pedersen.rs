...
mod test {
    ...
    use super::*;
    use crate::{constraint_system::{ecc::Point, StandardComposer, Variable}, prelude::{verify_proof, Circuit, Error, VerifierData}, util};
    use ark_bls12_377::Bls12_377;
    use ark_bls12_381::Bls12_381;
    use ark_ec::{
        twisted_edwards_extended::GroupAffine, AffineCurve, PairingEngine, ProjectiveCurve, TEModelParameters
    };
        ...
        P: TEModelParameters<BaseField = F>,
    {
        ...
        fn gadget(
            ...
            let ck_var_vec: Vec<Point<P>> = self.ck.iter().map(|ck| composer.add_affine(*ck)).collect();
            let msg_var_vec: Vec<Variable> = self.msg.iter().map(|m| composer.add_input(util::from_embedded_curve_scalar::<F, P>(*m))).collect(); 
            let open_var_vec: Vec<Variable> = self.open.iter().map(|o| composer.add_input(util::from_embedded_curve_scalar::<F, P>(*o))).collect();

            for i in 0..N {
                let p1: Point<P> = composer.variable_base_scalar_mul(msg_var_vec[i], ck_var_vec[0]);
                let p2: Point<P> = composer.variable_base_scalar_mul(open_var_vec[i], ck_var_vec[1]);
        ...
        VerifierData<F, PC>: PartialEq,
    {
        ...
        let msg: Vec<P::ScalarField> = (0..N).map(|_| P::ScalarField::rand(&mut test_rng())).collect();
        let open: Vec<P::ScalarField> = (0..N).map(|_| P::ScalarField::rand(&mut test_rng())).collect();
        
        let cm: Vec<GroupAffine<P>> = msg.iter().zip(open.iter()).map(|(m, o)| 
            (AffineCurve::mul(&ck[0], m.into_repr()) + 
            AffineCurve::mul(&ck[1], o.into_repr())).into_affine()
        ).collect();
