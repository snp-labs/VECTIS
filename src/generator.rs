    ...
    QAP: R1CStoQAP,
{
    ...
    for i in 0..(commit_witness_count as usize) {
        link_m.insert_row_slice(i, i, link_gens.pedersen_gens.clone()[..1].to_vec())?; // proof dependent
        link_m.insert_row_slice(
            i,
            commit_witness_count as usize,
            link_gens.pedersen_gens[1..].to_vec(),
        )?; // proof dependent
    }
